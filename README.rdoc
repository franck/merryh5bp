...
The setup rake tasks is :
- removing javascripts protypes files
- replacing the content of rails default layout application.html.erb by the content of index.html from html5-boilerplate. We then add several rails stuffs (csrf_meta_tag, yield in the main div, yield(:head) in the <head>, yield(:foot) at the bottom). We replace the css and js folder name of H5BP by the Rails naming style : stylesheets/ and javascripts/
- copying H5BP css and js assets in the public directory (we keep application.js instead of script.js). 
